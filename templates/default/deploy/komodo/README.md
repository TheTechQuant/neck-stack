...
The internal Caddy service uses a single public host from `DOMAIN`: Nuxt at `/`, Encore at `/api`, shared NECK Dash at `/__neck_dash` with its API at `/__neck_dash/api`, and the real SigNoz UI at `/__signoz`. Only `/__neck_dash/api/trace` bypasses dashboard Basic Auth so backend trace exporters can post there; NECK Dash still validates Encore trace auth. `pnpm komodo:setup` merges this app's `app_id=trace_key` pair into the shared stack's `NECKDASH_TRACE_AUTH_KEYS`. Override `NECK_DASH_PASSWORD_HASH` before deploy if you do not want to use the scaffolded first password.
