...
- `deploy`: Caddy, generated app Compose, shared SigNoz/NECK Dash Compose, Encore infra config, Komodo resources, and migration image.
- `scripts`: one zx command runner (`pnpm neck ...`) plus the small deploy generators it calls.
...
Advanced tasks stay behind the single runner:

```bash
pnpm neck help
pnpm neck api
pnpm neck infra
pnpm neck docker all
```
...
- `docs/openapi.json`, generated by `pnpm neck api openapi`
